...
def reynolds_calcs(length: float, v: float, KV: float) -> float:
    """
    Calculates the reynolds number of the ship based on the length and speed of the ship as well as the kinematic viscocity of water
    """
    return length * v / KV
...
def j_calcs(v, w, n, dProp)->float:
    ...
    advanced speed formula derived from rearranging the following formula for wake fraction coefficient: w = (v - v_a) / v
...
def etao_calcs(K_T: float, K_Q: float, J: float)->float:
    """
    calculates eta_o, the propeller efficiency, based on the thrust and torque coefficients and the advance ratio
    """
...
def HoltropMennenPowerCalculation(length, beam, T, displacementMass, v,
                                  ...
                                  numBlades = 3, n = 3, propKeelClearance = 0.2, pdRatio = 0.7):
    """
    ...
    pdRatio: the pitch-diameter ratio.
    """
    ####### INPUT VALUES ############
    ...
    J = j_calcs(v, w, n, dProp)
    print(f"J: {J}")
    J_actual = (v - v * 0.2584) / (1.6594 * 8)
    print(f"pitch actual:")
    print(f"J_actual: {J_actual}")
    print(f"AE/A0: {bladeAreaRatio}" )
    print(f"w: {w}")
    print(f"P/D: {pdRatio / dProp}")
    K_Q = bseries.torqueCoefficientCorrected(pdRatio, numBlades, bladeAreaRatio, J, reynolds)

    eta_o = etao_calcs(K_T, K_Q, J)
...
def main():
    samplePower = HoltropMennenPowerCalculation(205, 32, 10, 37500, 12.861, cM = 0.98, cWP = 0.75,
                                numPropellers = 1, dProp = 8,
                                numBlades = 4, n = 1.6594, pdRatio = 10)
    print(f"Sample Power: {samplePower}")
    """
